...
export const errorHandler = (err: Error, req: Request, res: Response, next: NextFunction) => {
    if (err instanceof AppError) {
        return res.status(err.status).send(new AppResponse(null, err))
    } else {
        return res.status(Errors.InternalServerError.status).send(new AppResponse(null, Errors.InternalServerError))
    }
}
